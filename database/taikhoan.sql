...
INSERT INTO nhomQuyen (maQuyen, ten)
...
INSERT INTO ctquyen (nhomQuyen, quyenHan) (SELECT nq.ma AS nhomQuyen, qh.ma AS quyenHan
                                           FROM quyenHan AS qh
...
#Them quyen them, sua, xoa cho sanpham, thuoctinh, nhapkho, khachhang, nhacungcap, nhanvien, nhomquyen
INSERT INTO quyenHan (chucNang, hanhDong) (SELECT cN.ma AS chucNang, hd.ma AS hanhDong
                                           FROM chucNang AS cN
                                                    INNER JOIN hanhdong AS hd
                                           WHERE cn.ma IN (14, 15, 16, 19, 20, 21, 22, 24)
                                             AND hd.ma IN (3, 4, 5));
...
SELECT qH.ma       AS maquyenHan,
       qH.chucNang AS maChucNang,
       cN.ten      AS tenChucNang,
       qH.hanhDong AS maHanhDong,
       hD.ten      AS tenHanhDong
FROM quyenHan AS qH
         INNER JOIN ptpm_taikhoan.chucNang cN on qH.chucNang = cN.ma
         INNER JOIN ptpm_taikhoan.hanhDong hD on qH.hanhDong = hD.ma
WHERE hD.ma IN (3, 4, 5);

SELECT *
FROM nhomQuyen;

INSERT INTO ctquyen (nhomQuyen, quyenHan) (SELECT nQ.ma, qH.ma
                                           FROM nhomQuyen nQ
                                                    INNER JOIN quyenHan qH
                                           WHERE nQ.ma = 1
                                             AND qH.hanhDong IN (3, 4, 5));

# Xem danh sach cac hanh dong
SELECT qh.ma AS maQuyenHan, cn.ma AS maChucNang, cn.ten AS tenChucNang, hd.ma AS maHanhDong, hd.ten AS tenHanhDong
FROM quyenHan AS qh
         INNER JOIN chucNang AS cn ON cn.ma = qh.chucNang
         INNER JOIN hanhDong AS hd ON hd.ma = qh.hanhDong;
...
ALTER TABLE taiKhoan
    ADD truyCap BOOLEAN DEFAULT FALSE;
